...
from pitchpulse.vaep_features.feature_dataset_loader import FeatureDataset
...
class TrainingDatasetNotReady(RuntimeError):
...
def build_preparation_training_manifest(
    *,
    ...
    dataset_adequate = bool(
        dataset_assessment.get("adequate_for_production_evaluation")
    )
    blockers = list(dataset_assessment.get("blockers", []))
    return {
        "schema_version": 1,
        "training_manifest_version": TRAINING_MANIFEST_VERSION,
        "stage": "data_preparation",
        "status": "ready_for_training" if dataset_adequate else "experimental",
        "source": {
        ...
        "ready_for_training": dataset_adequate,
        "training_blockers": blockers,
        "production_promotion": {
            "dataset_adequate": dataset_adequate,
            "allowed": False,
            "blockers": [
                *(f"dataset:{blocker}" for blocker in blockers),
                "models:not_trained_or_evaluated",
            ],
        },
    }


def require_training_dataset_ready(manifest: dict[str, Any]) -> None:
    if manifest.get("ready_for_training") is not True:
        blockers = manifest.get("training_blockers", [])
        raise TrainingDatasetNotReady(
