...
from .lineup_normalizer import StatsBombLineupNormalizer
from .three_sixty_normalizer import StatsBomb360Normalizer
from .postgres_writer import PostgresDataWriter

from .reader import RawDataFormatError, RawMatchBundle, StatsBombRawReader
from .validator import CanonicalEventValidator
from .lineup_validator import CanonicalLineupIntervalValidator
from .three_sixty_validator import Canonical360Validator
...
class StatsBombIngestionService:
    ...
    def _ingest_lineup_intervals(
        ...
        # StatsBomb occasionally repeats the same position interval in a lineup
        # payload. PostgreSQL collapses those rows on its natural key, so count
        # the extra source rows as deduplicated before writing. Otherwise the
        # raw/accepted/rejected/deduplicated audit fails to reconcile.
        unique_intervals = {
            (
                interval.source,
                interval.match_id,
        ...
        counts.deduplicated_lineup_intervals += (
            len(valid_intervals) - len(unique_intervals)
        )
        upserted = self.writer.upsert_player_match_intervals(
            ingestion_run_id, list(unique_intervals.values())
        )
