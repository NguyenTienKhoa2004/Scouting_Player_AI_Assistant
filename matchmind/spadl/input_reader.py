...
from matchmind.ingestion.normalizer import CanonicalEvent
from matchmind.ingestion.lineup_normalizer import CanonicalLineupInterval
from matchmind.ingestion.three_sixty_normalizer import Canonical360Frame
...
class PostgresSpadlInputReader:
    ...
    def validate_schema(self) -> tuple[ContractIssue, ...]:
        rows = self.connection.execute(
            ...
            """,
            (SILVER_SCHEMA, list(SPADL_INPUT_SCHEMA)),
        ).fetchall()
    ...
    def load(self, match_ids: Sequence[int] | None = None) -> SpadlInput:
        ...
        report = validate_spadl_input(events, intervals, frames)
    ...
    def _select_sql(
        ...
        return (
            f"SELECT {', '.join(columns)} FROM {SILVER_SCHEMA}.{table}{where} "
            f"ORDER BY {order_by}"
        )
...
__all__ = ["PostgresSpadlInputReader", "SILVER_SCHEMA", "SPADL_INPUT_SCHEMA"]
