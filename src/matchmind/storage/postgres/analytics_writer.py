"""PostgreSQL persistence for versioned Plan 03 analytics datasets."""
...
from matchmind.feature_engineering.action_state import STATE_CONTRACT_VERSION
from matchmind.feature_engineering.pipeline import AnalyticsDataset


class PostgresAnalyticsWriter:
    REQUIRED_TABLES = frozenset(
        {"analytics_runs", "analytics_actions", "analytics_action_features"}
    )
    ACTION_COLUMNS = (
    ...
    def write_dataset(self, run_id: int, dataset: AnalyticsDataset) -> None:
        ...
        self.connection.cursor().executemany(
            f"INSERT INTO analytics_actions ({columns}) VALUES ({placeholders})",
            action_rows,
        )

        self.connection.cursor().executemany(
            """
            INSERT INTO analytics_action_features (
