...
from matchmind.feature_engineering import (  # noqa: E402
...
class SpadlInputValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.validator = SpadlInputValidator()
    ...
    def test_rejects_duplicate_event_id_and_source_order(self) -> None:
        later = replace(
            self.event,
        ...
        report = self.validator.validate(
            [self.event, later, earlier_again],
            [self.interval, self.recipient_interval],
            [],
        )

        self.assertIn("duplicate_source_event_index", self._codes(report))
        self.assertIn("source_event_order_invalid", self._codes(report))
    ...
    def test_excluded_bad_behaviour_player_does_not_require_lineup(self) -> None:
        ...
        report = self.validator.validate([event], [], [])

        self.assertNotIn("event_player_missing_lineup_interval", self._codes(report))

    def test_rejects_subtype_that_does_not_match_preserved_source(self) -> None:
        event = replace(self.event, event_subtype="corner")

        report = self.validator.validate(
            [event], [self.interval, self.recipient_interval], []
        )

        self.assertIn("event_subtype_not_preserved", self._codes(report))

    def test_rejects_event_player_without_lineup_interval(self) -> None:
        report = self.validator.validate([self.event], [], [])
