...
class ValidatedFeatureDatasetLoaderTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_loads_a_reconciled_baseline(self) -> None:
        loaded = ValidatedFeatureDatasetLoader().load(
            self.artifact,
    ...
    def test_rejects_wrong_state_lineage(self) -> None:
        ...
        with self.assertRaisesRegex(
            FeatureDatasetValidationError, "state_contract_version"
        ):
            ValidatedFeatureDatasetLoader().load(self.artifact)

    def test_rejects_requested_lineage_that_does_not_match(self) -> None:
        loader = ValidatedFeatureDatasetLoader()

        with self.assertRaisesRegex(FeatureDatasetValidationError, "analytics run"):
            loader.load(self.artifact, expected_analytics_run_id=8)
        with self.assertRaisesRegex(
            FeatureDatasetValidationError, "dataset fingerprint"
        ):
    ...
    def test_rejects_non_reconciling_action_and_feature_keys(self) -> None:
        ...
        with self.assertRaisesRegex(FeatureDatasetValidationError, "do not reconcile"):
            ValidatedFeatureDatasetLoader().load(self.artifact)
    ...
    def test_rejects_any_column_outside_exact_feature_allowlist(self) -> None:
        ...
        with self.assertRaisesRegex(
            FeatureDatasetValidationError, "exact baseline allowlist"
        ):
            ValidatedFeatureDatasetLoader().load(self.artifact)
